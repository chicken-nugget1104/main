...
public class LevelRenderer implements LevelListener {
    ...
    public static final int MAX_REBUILDS_PER_FRAME = 7;
    ...
    public void render(int layer) {
        ...
        for (Chunk chunk : this.chunks) {
            ...
            // Render if bounding box of chunk is in frustum and is in draw distance or dd
    ...
    public void updateDirtyChunks(Player player) {
        ...
        if (!dirty.isEmpty()) {
            ...
            // Rebuild max 7 chunks per frame
            for (int i = 0; i < 7 && i < dirty.size(); i++) {
                dirty.get(i).rebuild();
            }
        }
    }
    ...
    public void renderHit(HitResult hitResult) {
        ...
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); //GL_CURRENT_BIT
        float alpha = ((float) Math.sin(System.currentTimeMillis() / 100.0D) * 0.2F + 0.4F) * 0.5F;
        glColor4f(1.0F, 1.0F, 1.0F, alpha);

        // Render faces
        this.tessellator.init();
        Tile.rock.renderFaceNoTexture(this.tessellator, hitResult.x, hitResult.y, hitResult.z, 0);
        Tile.rock.renderFaceNoTexture(this.tessellator, hitResult.x, hitResult.y, hitResult.z, 1);
        Tile.rock.renderFaceNoTexture(this.tessellator, hitResult.x, hitResult.y, hitResult.z, 2);
        Tile.rock.renderFaceNoTexture(this.tessellator, hitResult.x, hitResult.y, hitResult.z, 3);
        Tile.rock.renderFaceNoTexture(this.tessellator, hitResult.x, hitResult.y, hitResult.z, 4);
        Tile.rock.renderFaceNoTexture(this.tessellator, hitResult.x, hitResult.y, hitResult.z, 5);
        this.tessellator.flush();
